...
then
	...
	tar --exclude='*.tar.gz' --exclude='*.map' --exclude='*_refined_*' --exclude='*_group*' --append --file=$OUTPUTFOLDER/$PREFIX.tar $BASENAME/ref $BASENAME/contigs/alignmolvref/merge $BASENAME/contigs/alignmolvref/copynumber $BASENAME/contigs/exp_refineFinal1 $BASENAME/contigs/exp_refineFinal1_sv
	...
	tar --exclude='*.tar.gz' --exclude='*.map' --exclude='*_refined_*' --exclude='*_group*' --verbose --append --file=$OUTPUTFOLDER/$PREFIX.tar $BASENAME/ref $BASENAME/contigs/alignmolvref/merge $BASENAME/contigs/alignmolvref/copynumber $BASENAME/contigs/exp_refineFinal1 $BASENAME/contigs/exp_refineFinal1_sv
	tar --append --verbose --file=$OUTPUTFOLDER/$PREFIX.tar $BASENAME/contigs/exp_refineFinal1/alignmol/merge/EXP_REFINEFINAL1_merge.map
	echo "...done!"
	echo ""
